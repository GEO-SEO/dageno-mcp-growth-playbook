...
Docs / 文档: [README](https://github.com/dageno-agents/dageno-mcp-growth-playbook) · [简体中文](https://github.com/dageno-agents/dageno-mcp-growth-playbook/blob/main/README.zh-CN.md)
...
# Dageno MCP Growth Playbook 中文说明

> 把 Dageno API 和 MCP 数据变成增长报告、Prompt Gap 分析、引用情报和执行动作的实战手册。

## 它解决什么问题

- API 文档告诉你接口怎么调，但不一定告诉增长团队怎么把数据用起来。
- 这个项目把 Dageno API/MCP 能导出的数据、典型场景、Python client 和可执行 workflow 放在一起。

## 什么时候用它

- 你已经有 Dageno 项目和 API key。
- 你想把数据导出给 agent、BI、报告或自动化流程。
- 你要向内部团队解释 Dageno 数据可以做什么。

## 和相邻项目有什么区别

- 它是“API/MCP 使用手册 + 示例层”。
- `n8n-nodes-dageno` 是 n8n 的可视化自动化节点。
- `brand-ai-performance-check` 是把数据呈现成客户报告。
...
- example prompts
- growth workflows
- citation and prompt-gap examples

## 快速开始

1. 准备自己的 Dageno project 和 `x-api-key`。
2. 按 README 配置 REST/Python 或 MCP。
3. 从 reporting、prompt gap、citation intelligence 或 opportunity workflow 开始。
